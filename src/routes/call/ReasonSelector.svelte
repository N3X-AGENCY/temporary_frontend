    ...
    <div class="mt-6" in:slide={{duration: 200}}>
      ...
      <div class="grid grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
        {#each categories[selectedCategory as keyof typeof categories] as subcategory}
          <button
            type="button"
            class=" rounded-lg p-3 text-sm {selectedSubcategory === subcategory ? 'bg-white/0 text-black' : 'bg-white/10'} focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-30 text-white transition-all duration-200"
