...
<div class="bg-black bg-opacity-20 border border-white border-opacity-10 rounded-sm p-6" in:fade={{duration: 200}}>
  <h3 class="text-lg font-semibold mb-4 text-white">Reason for Call</h3>
  <div class="grid grid-cols-3 gap-3 mb-6">
    {#each Object.keys(categories) as category}
      <button
        type="button"
        class="p-3 text-sm rounded-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-30 text-white transition-all duration-200"
  ...
  {#if selectedCategory}
    <div class="mt-6" in:slide={{duration: 200}}>
      <h4 class="text-md font-semibold mb-3 text-white text-opacity-80">Select Subcategory</h4>
      <div class="grid grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
        {#each categories[selectedCategory as keyof typeof categories] as subcategory}
          <button
            type="button"
            class="p-3 text-sm rounded-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-30 text-white transition-all duration-200"
            class:bg-white={selectedSubcategory === subcategory}
            class:bg-opacity-20={selectedSubcategory === subcategory}
